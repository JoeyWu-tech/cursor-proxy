...
确保你的代理软件已启动，且有可用的本地代理端口（如 Clash 默认 `127.0.0.1:7890`）。

### Step 2：下载文件

从 Release 页面下载压缩包（默认 x64 版本），解压得到：
- `version.dll`
- `config.json`
...
- ✅ 多个窗口/标签页可以同时进行代码补全、Chat、Agent 等操作，不会互相卡死
- ✅ 流式输出（模型打字效果）更流畅，断流概率大幅降低
- ✅ 头部压缩减少冗余数据，整体延迟更低

## 注意事项

- ⚠️ 记得关闭杀毒软件或将 `version.dll` 加入白名单
- ⚠️ 如遇 `0xc0000142` 错误，请安装 [VC++ 运行库](https://aka.ms/vs/17/release/vc_redist.x64.exe)
- ⚠️ Cursor 更新后可能需要重新放置文件
